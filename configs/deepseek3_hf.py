JOB_NAME = "deepseekv3_hf"
DO_ALERT = False

SEQ_LEN = 2048

MODEL_ONLY_FOLDER = "/mnt/afs/caizheng/DeepSeek-V3-hf-ckpt-bf16-small-fixed"
...
ckpt = dict(
    ...
    # load_ckpt_info=dict(path="/mnt/afs/caizheng/InternEvo/llm_ckpts/100", content=("model",), ckpt_type="internevo"),
    load_ckpt_info=dict(path=MODEL_ONLY_FOLDER, content=("model",), ckpt_type="hh"),
    auto_resume=False,
...
data = dict(
    # type="streaming",
    # tokenizer_path="/mnt/afs/caizheng/tokenizer_deepseek_v2",
    seq_len=SEQ_LEN,
...
loss = dict(label_smoothing=0, op_type="py_vocab_parallel", mtp_loss_coeff=0.3)
...
hf = dict(
    cfg="huggingface_models.deepseek3_model.configuration_deepseek3",
    cfg_cls="DeepseekV3Config",
    ...
    mod="huggingface_models.deepseek3_model.modeling_deepseek3",
    mod_cls="DeepseekV3ForCausalLM",
)

fsdp_wrap_cls = [
    dict(
        mod = "huggingface_models.deepseek3_model.modeling_deepseek3",
        mod_cls = "DeepseekV3DecoderLayer",
    ),
]
...
"""
parallel = dict(
    fsdp=dict(enable=True, mode="v2", init_method="cuda"),
    zero1=dict(size=1),
    tensor=dict(size=1, mode="mtp"),
    pipeline=dict(size=1, interleaved_overlap=True),
    weight=dict(size=1, overlap=True, memory_pool=False),
    expert=dict(size=8),
    expert_weight=dict(size=1, overlap=True, memory_pool=False),
