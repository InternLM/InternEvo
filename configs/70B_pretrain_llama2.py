"""
基于internlm实现的llama2 70B模型的config，下面的配置可以在128张80GB-A800上运行
srun -p llm_s -n128 -N16 --ntasks-per-node=8 --gpus-per-task=1 python train.py --config configs/70B_pretrain_llama2.py
"""
...
TRAIN_FOLDER = None # 数据集路径，如果为None则会使用dummy data训练
VALID_FOLDER = None
