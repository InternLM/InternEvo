...
_CONFIG_FOR_DOC = "InternLMMoEConfig"
...
def _compute_load_balancing_loss(gate_logits: torch.Tensor, num_experts: torch.Tensor = None, top_k=2) -> float:
    """Calculate the load balancing loss contribution."""
    if gate_logits is None or not isinstance(gate_logits, tuple) or gate_logits[0] is None:
        return 0
    moe_losses = []
    for logit in gate_logits:
        gates = F.softmax(logit, dim=1)
        weight, indices = torch.topk(gates, top_k, dim=1)
        num_tokens_per_expert = torch.histc(indices, bins=num_experts, min=0, max=num_experts)
        moe_losses.append(torch.dot(num_tokens_per_expert.to(weight.dtype), weight.mean(dim=0)))
...
class InternLMMoELayer(nn.Module):
    def __init__(self, config: InternLMMoEConfig):
        super().__init__()
        self.config = config
        ...
        if config.num_shared_experts > 0:
            ...
            self.shared_experts = InternLMMLP(
                hidden_size=config.hidden_size,
    ...
    def forward(self, x):
        ...
        weights, indices = torch.topk(gates, self.num_experts_per_tok, dim=1)
        ...
        for i, expert in enumerate(self.experts):
            y[flat_indices == i] = expert(x[flat_indices == i])
        ...
        # moe_loss = self.load_balancing_loss(weights, indices) if self.training else None
        return y.view(*orig_shape), logits
...
class InternLMMoEDecoderLayer(nn.Module):
    def __init__(self, config: InternLMMoEConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class InternLMMoEPreTrainedModel(PreTrainedModel):
    config_class = InternLMMoEConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["InternLMMoEDecoderLayer"]
    ...
    def _set_gradient_checkpointing(self, module, value=False):  # pylint: disable=W0237
        if isinstance(module, InternLMMoEModel):
...
class InternLMMoEModel(InternLMMoEPreTrainedModel):
    """
    ...
    def __init__(self, config: InternLMMoEConfig):
        ...
        self.layers = nn.ModuleList([InternLMMoEDecoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class InternLMMoEForCausalLM(InternLMMoEPreTrainedModel):
    _auto_class = "AutoModelForCausalLM"

    def __init__(self, config):
        super().__init__(config)
        self.model = InternLMMoEModel(config)
    ...
    def forward(
        ...
        moe_loss = None
        if output_router_logits:
            aux_loss = _compute_load_balancing_loss(
    ...
    @torch.no_grad()
    def stream_chat(  # pylint: disable=W0102
        self,
        tokenizer,
        query: str,
        history: List[Tuple[str, str]] = [],
...
class InternLMMoEForSequenceClassification(InternLMMoEPreTrainedModel):
        ...
        if labels is not None:
            ...
            if self.config.problem_type is None:
                if self.num_labels == 1:
                    ...
                    labels.dtype == torch.long or labels.dtype == torch.int  # pylint: disable=R1714
                ):
