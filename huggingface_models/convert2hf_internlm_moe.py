...
from internlm_moe_model import InternLMMoEConfig, InternLMMoEForCausalLM
...
def load_source(src):
    ...
    num_moe_layer = model_config["num_layers"]
    ...
    moe_states = [[{} for _ in range(max_pp)] for __ in range(max_tp)]
    for tp in tqdm(range(max_tp)):
        for pp in tqdm(range(max_pp)):
            ...
            for lay_id in tqdm(range(num_moe_layer)):
                for expert_id in range(num_experts):
                    moe_ckpt_name = os.path.join(src, f"model_moe_layer{lay_id}_expert{expert_id}_tp{tp}.pt")
                    moe_states[tp][pp].update(torch.load(moe_ckpt_name, map_location="cpu"))
    print("Source Checkpoint Loaded", flush=True)
    return model_config, states, moe_states
...
def convert(src, tgt, tokenizer, dtype, max_shard_size, max_pos, topk, rope_scaling):
    """
    ...
    """
    # load states
    model_config, src_states, src_moe_states = load_source(src)
    states = merge(src_states)
    moe_states = merge(src_moe_states)
    del src_states
    del src_moe_states
    ...
    for layer_i in tqdm(range(model_config["num_layers"])):
        ...
        state_dict[f"model.layers.{layer_i}.mlp.gate.weight"] = states[0][
            f"blocks.{layer_i}.mlp.moe_layer.gate.wg.weight"
        ].clone()

        if model_config["moe_use_residual"]:
            state_dict[f"model.layers.{layer_i}.mlp.shared_experts.gate_proj.weight"] = torch.cat(
                [moe_states[i][f"blocks.{layer_i}.mlp.moe_layer.residual_mlp.w1.weight"] for i in range(num_shards)],
                dim=0,
            )
            state_dict[f"model.layers.{layer_i}.mlp.shared_experts.down_proj.weight"] = torch.cat(
                [moe_states[i][f"blocks.{layer_i}.mlp.moe_layer.residual_mlp.w3.weight"] for i in range(num_shards)],
                dim=1,
            )
            state_dict[f"model.layers.{layer_i}.mlp.shared_experts.up_proj.weight"] = torch.cat(
                [moe_states[i][f"blocks.{layer_i}.mlp.moe_layer.residual_mlp.w2.weight"] for i in range(num_shards)],
                dim=0,
            )

        for expert_id in range(model_config["num_experts"]):
            state_dict[f"model.layers.{layer_i}.mlp.experts.{expert_id}.gate_proj.weight"] = torch.cat(
                [
                    moe_states[i][f"blocks.{layer_i}.mlp.moe_layer.experts.wrapped_experts.{expert_id}.w1.weight"]
                    for i in range(num_shards)
                ],
                dim=0,
            )
            state_dict[f"model.layers.{layer_i}.mlp.experts.{expert_id}.down_proj.weight"] = torch.cat(
                [
                    moe_states[i][f"blocks.{layer_i}.mlp.moe_layer.experts.wrapped_experts.{expert_id}.w3.weight"]
                    for i in range(num_shards)
                ],
                dim=1,
            )
            state_dict[f"model.layers.{layer_i}.mlp.experts.{expert_id}.up_proj.weight"] = torch.cat(
                [
                    moe_states[i][f"blocks.{layer_i}.mlp.moe_layer.experts.wrapped_experts.{expert_id}.w2.weight"]
                    for i in range(num_shards)
                ],
                dim=0,
            )

        intermediate_size, _ = state_dict[f"model.layers.{0}.mlp.experts.{0}.gate_proj.weight"].shape
    ...
    config = InternLMMoEConfig(
        ...
        num_shared_experts=1 if model_config["moe_use_residual"] else 0,
    )
    # tokenizer
    ...
    with no_init_weights():
        model = InternLMMoEForCausalLM._from_config(config, torch_dtype=dtype)
    ...
    X = torch.zeros((32, 32), dtype=torch.int64).to(device=device)
    labels = []
    for i in range(32):
        labels.append((i + 1) % 32)
        X[i] = 1
    labels = torch.tensor(labels)
    dataset = Dataset.from_dict({"input_ids": X, "labels": X})
    ...
    config_dict["auto_map"]["AutoModel"] = "modeling_internlm.InternLMMoEForCausalLM"
...
def parse_args():
    ...
    parser.add_argument("--topk", type=int, default=1, help="top-k experts in MoE.")
...
if __name__ == "__main__":
    ...
    assert args.topk is not None, "--topk is needed!"
    start = time.time()
    convert(args.src, args.tgt, args.tokenizer, dtype, args.max_shard, args.max_pos, args.topk, rope_scaling)
