# Adapted from https://github.com/NVIDIA/TransformerEngine/blob/v1.12/transformer_engine/pytorch/cpu_offload.py
...
class SynchronizedGroupOffloadHandler(OffloadHandler):
    ...
    def __init__(self, num_offload_group, tensor_need_offloading_checker=(lambda _: True), debug=False) -> None:
        super().__init__()
...
class AsyncDoubleBufferGroupOffloadHandler(SynchronizedGroupOffloadHandler):
    ...
    def __init__(
        ...
        if torch.distributed.get_rank() == 0:
            print(
                f"Offloading {self.num_offload_group} layers' activations with "
                f"layer_window_map:{self.layer_window_map}",
                flush=True,
            )
    ...
    def synchronize_on_group_commit_forward(self, current_group):
        ...
        # of layers offloaded
        # e.g. layer_window_map={0: 10, 1: 21, 2: 31}
...
def get_cpu_offload_context(
    ...
    offload_activations: bool, default = `False`
                         When set to `True`, offloads the tensors with attribute 'activation_offloading' for the layer.
    offload_weights: bool, default = `False`
                     When set to `True`, offloads the weights with attribute 'weight_offloading' for the layer.
    """

    def tensor_need_offloading_checker_base(tensor):  # pylint: disable=W0613
        return True
    ...
    if offload_activations and offload_weights:
        ...
        tensor_need_offloading_checker = tensor_need_offloading_checker_base
