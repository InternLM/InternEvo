        ...
        if self.prenorm:
            ...
            if not isinstance(self.feed_forward, nn.Identity):
                ...
                if self.num_experts <= 1:  # dense mlp output
                    ...
                    hidden_states, gate_logits = self.feed_forward(hidden_states)

            return hidden_states + residual, gate_logits
        else:
            ...
            if not isinstance(self.feed_forward, nn.Identity):
                if self.num_experts <= 1:  # dense mlp output
                    ...
                    mlp_out, gate_logits = self.feed_forward(hidden_states)
...
class Qwen3Moe(BaseModel):
    ...
    def forward(self, hidden_states=None, input_ids=None, **kwargs):
        ...
        for _, block in enumerate(self.layers):
            hidden_states, gate_logits = block(
                hidden_states,
                residual=None,
                **kwargs,
            )
            ...
            if gate_logits is not None:
                layer_gate_logits.append(gate_logits)
        ...
        if extra_hidden_states_list is not None:
            return (hidden_states, extra_hidden_states_list), [l_aux]
    ...
    def load_balancing_loss_func(
        ...
        if not use_attn_mask:
            ...
            attention_mask = attention_mask.reshape((micro_bsz, seqlens))

            # Compute the mask that masks all padding tokens as 0 with the same shape of expert_mask
            expert_attention_mask = (
                attention_mask[None, :, :, None, None]
                .expand((num_hidden_layers, micro_bsz, seqlens, self.top_k, self.num_experts))
            ...
            router_per_expert_attention_mask = (
                ...
                .expand((num_hidden_layers, micro_bsz, seqlens, self.num_experts))
        ...
        l_aux = torch.sum(tokens_per_expert * router_prob_per_expert.unsqueeze(0)) * self.num_experts

        return l_aux

    def split_cuseqlens(self, cu_seqlens: torch.Tensor, max_seqlen: int):
        n = gpc.get_world_size(ParallelMode.TENSOR)
        if n == 1:
            return cu_seqlens
