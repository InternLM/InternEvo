...
from internlm.model.moe.megablock.utils import act_fn, dsd_nn, sdd_nt
...
class MegaBlockFeedForward(nn.Module):
    ...
    def forward(self, x, topo):
        ...
        w1_o = sdd_nt(x, self.w1, topo, gpc.get_group(ParallelMode.TENSOR), self.tp_mode)
        w3_o = sdd_nt(x, self.w3, topo, gpc.get_group(ParallelMode.TENSOR), self.tp_mode)
        out = dsd_nn(act_fn(w1_o, w3_o, topo), self.w2, gpc.get_group(ParallelMode.TENSOR), self.tp_mode)
