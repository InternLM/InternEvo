...
from .meta_type import STATE_DICT_TYPE
...
from .vescale_planner import VeScaleSavePlanner
...
def save_state_dict(
    ...
    def local_step():
        ...
        # print(f"local_plan {gpc.get_global_rank()} {gpc.get_local_rank(ParallelMode.TENSOR)}: {local_plan}", flush=True)
        return local_plan
    ...
    def write_data(async_io: bool = False, io_workers=io_workers):
        ...
        assert planner is not None
        # print(f"central_plan {gpc.get_global_rank()} {gpc.get_local_rank(ParallelMode.TENSOR)}: {central_plan}", flush=True)
    ...
    def finish_checkpoint(all_results):
        ...
        print(f"global_metatadata: {global_metatadata}", flush=True)
