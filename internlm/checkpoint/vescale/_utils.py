...
from .placement_types import InterleavedShard, Partial, Placement, Replicate, Shard
from ._collective_utils import mesh_all_gather
...
def compute_global_tensor_info(
    ...
    for idx, placement in enumerate(placements):
        ...
        if placement.is_shard() or placement.is_interleaved_shard():
            ...
            is_contiguous_tensor = all(tensor_stride[i] >= tensor_stride[i + 1] for i in range(len(tensor_stride) - 1))
            for i in range(len(tensor_stride)):
                if (i != shard_dim and tensor_stride[i] >= tensor_stride[shard_dim] and not is_contiguous_tensor) or (
                    i < shard_dim and is_contiguous_tensor
                ):
                    ...
                    if meshdim_localtensor_shape is None or is_shard_same_dim:
                        ...
                        if local_dim_size == 0:
                            tensor_stride[i] *= max(global_dim_size, 1)
                        else:
