...
from ._utils import compute_local_shape, compute_local_offset
...
def _create_write_item_for_tensor(fqn: str, tensor: torch.Tensor, is_optimizer=False) -> WriteItem:
    ...
    #     assert child.weight.complete_size[0] % k == 0 and child.weight.complete_size[0] // k == gpc.get_world_size(ParallelMode.TENSOR), f"{child.weight.complete_size}, {child.weight.offset}"
    # if child.weight.offset[1] != 0:
    #     k = child.weight.offset[1] // gpc.get_local_rank(ParallelMode.TENSOR)
    #     assert child.weight.complete_size[1] % k == 0 and child.weight.complete_size[1] // k == gpc.get_world_size(ParallelMode.TENSOR), f"{child.weight.complete_size}, {child.weight.offset}"
...
def find_state_dict_object(state_dict: STATE_DICT_TYPE, index: MetadataIndex, fqn=None) -> Any:
